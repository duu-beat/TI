            ...
            <div class="space-y-8">
                <div>
                    <h1 class="text-4xl md:text-5xl font-black text-white tracking-tight mb-6">
                        {{ __('messages.public.contact_title') }}<br>
                        <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">{{ __('messages.public.contact_title_highlight') }}</span>
                    </h1>
                    <p class="text-lg text-slate-400 leading-relaxed">
                        {{ __('messages.public.contact_description') }}
                    </p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition">
                        ...
                        <div>
                            <div class="text-xs font-bold text-slate-500 uppercase">{{ __('messages.public.email_label') }}</div>
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition">
                        ...
                        <div>
                            <div class="text-xs font-bold text-slate-500 uppercase">{{ __('messages.public.phone_label') }}</div>
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition">
                        ...
                        <div>
                            <div class="text-xs font-bold text-slate-500 uppercase">{{ __('messages.public.location_label') }}</div>
                ...
                @auth
                    ...
                    @if(auth()->user()->isMaster())
                        <div class="flex flex-col items-center justify-center text-center h-full min-h-[400px] space-y-6">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-white mb-2">{{ __('messages.public.security_panel') }}</h3>
                                <p class="text-slate-400 max-w-xs mx-auto text-sm">
                                    {{ __('messages.public.security_panel_text') }}
                                </p>
                            </div>
                    ...
                    @elseif(auth()->user()->isAdmin())
                        <div class="flex flex-col items-center justify-center text-center h-full min-h-[400px] space-y-6">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-white mb-2">{{ __('messages.public.admin_panel') }}</h3>
                                <p class="text-slate-400 max-w-xs mx-auto text-sm">
                                    {{ __('messages.public.admin_panel_text') }}
                                </p>
                            </div>
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center text-center h-full min-h-[400px] space-y-6">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-white mb-2">{{ __('messages.public.already_client') }}</h3>
                                <p class="text-slate-400 max-w-xs mx-auto text-sm">
                                    {{ __('messages.public.client_contact_text') }}
                                </p>
                            </div>
                ...
                @else
                    ...
                    <form action="{{ route('contact.submit') }}" method="POST" class="space-y-5 relative z-10">
                        ...
                        <div class="grid md:grid-cols-2 gap-5">
                            <div class="space-y-1">
                                <label class="text-xs font-bold text-slate-400 uppercase ml-1">{{ __('messages.public.name_label') }}</label>
                                <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-xl border border-white/10 bg-slate-950/50 px-4 py-3 text-slate-100 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition" placeholder="{{ __('auth.ui.name') }}" required>
                                @error('name') <span class="text-xs text-red-400 ml-1">{{ $message }}</span> @enderror
                            </div>
                            <div class="space-y-1">
                                <label class="text-xs font-bold text-slate-400 uppercase ml-1">{{ __('messages.public.phone_label_short') }}</label>
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-bold text-slate-400 uppercase ml-1">{{ __('messages.public.message_label') }}</label>
                            <textarea name="message" rows="4" class="w-full rounded-xl border border-white/10 bg-slate-950/50 px-4 py-3 text-slate-100 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition resize-none" placeholder="{{ __('messages.public.message_placeholder') }}" required>{{ old('message') }}</textarea>
                        ...
                        <button type="submit" class="w-full py-4 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold shadow-lg shadow-cyan-500/20 hover:scale-[1.02] transition">
                            {{ __('messages.public.send_message') }}
                        </button>
                    </form>
                @endauth
            </div>
