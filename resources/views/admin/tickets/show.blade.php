            ...
            <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="text-[11px] text-slate-500">{{ __('tickets.ui.created') }} {{ $ticket->created_at->diffForHumans() }}</span>
            ...
            <div class="grid gap-6 lg:grid-cols-12 lg:gap-7">
                <section class="space-y-6 lg:col-span-8" aria-label="Detalhes e histórico técnico">
                        ...
                        <header class="relative flex flex-col gap-4 border-b border-white/5 bg-white/[0.025] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                            <div class="flex min-w-0 items-center gap-3.5">
                                <div class="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-slate-800 text-base font-black text-white shadow-inner">
                                    ...
                                    <span class="absolute -bottom-1.5 -right-1.5 rounded-md border border-indigo-400/25 bg-slate-950 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider text-indigo-200">{{ __('tickets.ui.client') }}</span>
                                </div>
                            ...
                            <div class="rounded-xl border border-white/10 bg-slate-950/35 px-3 py-2 text-xs text-slate-400"><span class="text-slate-500">{{ __('tickets.ui.updated') }}</span> {{ $ticket->updated_at->diffForHumans() }}</div>
                        </header>
                        <div class="relative p-5 sm:p-7">
                            <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-indigo-300">{{ __('tickets.ui.request') }}</p>
                            ...
                            @if($ticket->messages->isNotEmpty() && $ticket->messages->first()->attachments->isNotEmpty())
                                <div class="mt-6 border-t border-white/5 pt-5"><div class="mb-3 flex items-center gap-2"><svg class="h-4 w-4 text-cyan-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="m13.5 10.5-3 3m0 0-3-3m3 3v-9m5.5 5.5v6A2.5 2.5 0 0 1 13.5 19h-7A2.5 2.5 0 0 1 4 16.5v-6" /></svg><h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('tickets.ui.evidence') }}</h3></div><div class="flex flex-wrap gap-3">@foreach($ticket->messages->first()->attachments as $attachment)<x-ticket-attachment-card :attachment="$attachment" compact />@endforeach</div></div>
                            @endif
                        </div>
                    </article>
                    ...
                    <section id="admin-ticket-timeline" class="rounded-3xl border border-white/10 bg-slate-900/55 p-5 shadow-xl shadow-slate-950/20 backdrop-blur-xl sm:p-7" aria-labelledby="admin-ticket-timeline-title">
                        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 pb-5"><div><p class="text-[11px] font-bold uppercase tracking-[0.18em] text-indigo-300">{{ __('tickets.ui.timeline') }}</p><h2 id="admin-ticket-timeline-title" class="mt-1 text-lg font-bold text-white">{{ __('tickets.ui.communication_notes') }}</h2></div><div class="flex items-center gap-2 text-[11px] text-slate-500"><span class="h-2 w-2 rounded-full bg-indigo-400"></span>{{ __('tickets.ui.public') }} <span class="ml-2 h-2 w-2 rounded-full bg-amber-400"></span>{{ __('tickets.ui.internal') }}</div></div>
                        <div class="relative mt-6 space-y-5 before:absolute before:bottom-4 before:left-5 before:top-4 before:w-px before:bg-white/10 sm:before:left-6">
                                ...
                                <article x-show="index > 0" class="relative flex gap-3 sm:gap-4" :class="message.user_id == {{ auth()->id() }} && !message.is_internal ? 'sm:flex-row-reverse' : ''">
                                    ...
                                    <div class="min-w-0 flex-1" :class="message.user_id == {{ auth()->id() }} && !message.is_internal ? 'sm:text-right' : ''">
                                        <div class="inline-block w-full max-w-3xl rounded-2xl border p-4 text-left shadow-lg sm:p-5" :class="message.is_internal ? 'border-amber-400/20 bg-amber-950/30 sm:rounded-tl-md' : (message.user_id == {{ auth()->id() }} ? 'border-indigo-400/20 bg-indigo-500/10 sm:rounded-tr-md' : 'border-white/10 bg-slate-950/45 sm:rounded-tl-md')">
                                            <div class="mb-3 flex items-center justify-between gap-3" :class="message.user_id == {{ auth()->id() }} && !message.is_internal ? 'sm:flex-row-reverse' : ''"><div class="flex min-w-0 items-center gap-2"><span class="truncate text-xs font-bold" :class="message.is_internal ? 'text-amber-200' : 'text-white'" x-text="message.user?.name || '{{ __('tickets.ui.system') }}'"></span><template x-if="message.is_internal"><span class="rounded-md border border-amber-400/20 bg-amber-400/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-200">{{ __('tickets.ui.internal_note') }}</span></template><template x-if="!message.is_internal && (message.user?.role === 'admin' || message.user?.role === 'master')"><span class="rounded-md border border-cyan-400/20 bg-cyan-400/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-cyan-200">{{ __('tickets.ui.team') }}</span></template></div><time class="shrink-0 text-[10px] text-slate-500" x-text="new Date(message.created_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })"></time></div>
                                            <div class="text-sm leading-6 text-slate-300" x-html="message.message.replace(/\n/g, '<br>')"></div>
                                            <template x-if="message.is_internal && message.time_spent"><div class="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-amber-400/15 bg-amber-400/[0.06] px-2 py-1 text-[10px] font-semibold text-amber-200"><svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg><span x-text="message.time_spent + ' {{ __('tickets.ui.minutes_registered') }}'"></span></div></template>
                            ...
                            <p x-show="messages.length <= 1" class="rounded-xl border border-dashed border-white/10 bg-slate-950/30 px-4 py-5 text-center text-sm text-slate-500">{{ __('tickets.ui.no_interactions') }}</p>
                        </div>
                    </section>

                    @if($ticket->rating)
                        <section class="rounded-3xl border border-emerald-400/20 bg-emerald-500/[0.06] p-5 sm:p-6"><div class="flex gap-3"><div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-400/10 text-emerald-300">★</div><div><p class="text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-300">{{ __('tickets.ui.customer_rating') }}</p><div class="mt-1 flex text-lg text-amber-300">@foreach(range(1, 5) as $i)<span class="{{ $i <= $ticket->rating ? '' : 'text-slate-700' }}">★</span>@endforeach</div>@if($ticket->rating_comment)<p class="mt-2 text-sm italic text-slate-400">“{{ $ticket->rating_comment }}”</p>@endif</div></div></section>
                    @endif
                </section>
                ...
                <aside class="space-y-5 lg:col-span-4" aria-label="Painel de operação do chamado">
                    <section class="rounded-3xl border border-white/10 bg-slate-900/75 p-5 shadow-xl shadow-slate-950/20 backdrop-blur-xl sm:p-6 lg:sticky lg:top-6">
                        <div class="flex items-center justify-between border-b border-white/5 pb-4"><div><p class="text-[10px] font-bold uppercase tracking-[0.16em] text-indigo-300">{{ __('tickets.ui.operation') }}</p><h2 class="mt-1 text-sm font-bold text-white">{{ __('tickets.ui.ticket_control') }}</h2></div><span class="rounded-lg border border-white/10 bg-slate-950/35 px-2 py-1 text-[10px] font-mono text-slate-500">#{{ $ticket->id }}</span></div>
                        <form action="{{ route('admin.tickets.update-status', $ticket) }}" method="POST" class="mt-5 space-y-3">@csrf @method('PATCH')<div><label for="ticket-status" class="mb-2 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Status</label><select id="ticket-status" name="status" class="block w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2.5 text-sm text-slate-200 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20">@foreach(\App\Enums\TicketStatus::cases() as $status)<option value="{{ $status->value }}" {{ $ticket->status === $status ? 'selected' : '' }} class="bg-slate-900">{{ $status->label() }}</option>@endforeach</select></div><button type="submit" class="inline-flex w-full items-center justify-center rounded-xl bg-indigo-500 px-4 py-2.5 text-xs font-bold text-white shadow-lg shadow-indigo-500/15 transition hover:bg-indigo-400">{{ __('tickets.ui.save_status') }}</button></form>
                        <form action="{{ route('admin.tickets.assign', $ticket) }}" method="POST" class="mt-5 border-t border-white/5 pt-5">@csrf @method('PATCH')<label for="ticket-assignee" class="mb-2 block text-[10px] font-bold uppercase tracking-wider text-slate-500">{{ __('tickets.ui.assignee') }}</label><div class="flex gap-2"><select id="ticket-assignee" name="assigned_to" class="min-w-0 flex-1 rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2.5 text-xs text-slate-200 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option value="">{{ __('tickets.ui.unassigned') }}</option>@foreach($admins ?? [] as $admin)<option value="{{ $admin->id }}" {{ $ticket->assigned_to == $admin->id ? 'selected' : '' }} class="bg-slate-900">{{ $admin->name }}</option>@endforeach</select><button type="submit" class="rounded-xl border border-white/10 bg-white/5 px-3 text-slate-300 transition hover:bg-white/10 hover:text-white" aria-label="{{ __('tickets.ui.save_assignee') }}"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m5 12 4 4L19 6" /></svg></button></div></form>
                        <div class="mt-5 grid gap-2 border-t border-white/5 pt-5 sm:grid-cols-2"><a href="{{ route('admin.visits.create', $ticket) }}" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-cyan-400/20 bg-cyan-500/[0.07] px-3 py-2.5 text-xs font-bold text-cyan-200 transition hover:bg-cyan-500/15"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M8 7V3m8 4V3M4 11h16M5 5h14a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Z" /></svg>{{ __('tickets.ui.visit') }}</a><div x-data="{ open: false }" class="relative"><button type="button" @click="open = !open" class="inline-flex w-full items-center justify-center rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-xs font-bold text-slate-300 transition hover:bg-white/10 hover:text-white">{{ __('tickets.ui.merge') }}</button><div x-show="open" x-cloak @click.outside="open = false" class="absolute bottom-full right-0 z-30 mb-2 w-60 rounded-2xl border border-white/10 bg-slate-900 p-3 shadow-2xl"><p class="mb-2 text-[11px] text-slate-400">{{ __('tickets.ui.merge_target_hint') }}</p><form action="{{ route('admin.tickets.merge', $ticket) }}" method="POST" onsubmit="return confirm('{{ __('tickets.ui.merge_confirm') }}');">@csrf<input type="number" name="target_ticket_id" required class="block w-full rounded-lg border border-white/10 bg-slate-950 px-2.5 py-2 text-xs text-white focus:border-indigo-400 focus:outline-none" placeholder="{{ __('tickets.ui.ticket_id_placeholder') }}"><button type="submit" class="mt-2 w-full rounded-lg bg-indigo-500 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-400">{{ __('tickets.ui.confirm_merge') }}</button></form></div></div></div>
                        @if(!$ticket->is_escalated)<form action="{{ route('admin.tickets.escalate', $ticket) }}" method="POST" class="mt-2" onsubmit="return confirm('{{ __('tickets.ui.escalate_confirm') }}');">@csrf<button type="submit" class="inline-flex w-full items-center justify-center gap-1.5 rounded-xl border border-rose-400/20 bg-rose-500/[0.07] px-3 py-2.5 text-xs font-bold text-rose-300 transition hover:bg-rose-500/15"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M5.07 19h13.86c1.54 0 2.5-1.67 1.73-3L13.73 4c-.77-1.33-2.69-1.33-3.46 0L3.34 16c-.77 1.33.19 3 1.73 3Z" /></svg>{{ __('tickets.ui.escalate_security') }}</button></form>@endif
                    </section>
                    ...
                    <section class="rounded-3xl border border-white/10 bg-slate-900/65 p-5 shadow-xl shadow-slate-950/20 backdrop-blur-xl sm:p-6">
                        <div class="border-b border-white/5 pb-4"><p class="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">{{ __('tickets.ui.inventory') }}</p><h2 class="mt-1 text-sm font-bold text-white">{{ __('tickets.ui.customer_equipment') }}</h2></div>
                        ...
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-white/5 bg-slate-950/35 px-3 py-2.5">
                            ...
                            <div class="relative flex-1 max-w-xs">
                                <input type="text" x-model="wikiQuery" @input.debounce.300ms="searchWiki()" @focus="showWiki = true"
                                       ...
                                       placeholder="{{ __('tickets.ui.search_wiki_placeholder') }}">
                                ...
                                <div x-show="showWiki && wikiResults.length > 0" @click.away="showWiki = false" x-cloak
                                    ...
                                    <div class="p-2 bg-slate-900/50 border-b border-white/5 text-[9px] font-bold text-slate-500 uppercase tracking-widest">{{ __('tickets.ui.wiki_suggestions') }}</div>
