<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button @click="$dispatch('open-create-modal')" 
                    class="group flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 border border-indigo-400/30 rounded-xl text-sm font-bold text-white transition hover:shadow-[0_0_20px_rgba(99,102,241,0.4)]">
    ...
    >
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- SKELETON LOADER --}}
            <div x-show="!loaded" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
                @for($i = 0; $i < 6; $i++)
                <div class="bg-white/5 border border-white/5 rounded-2xl p-6 h-48 flex flex-col justify-between">
            ...
            <div x-show="loaded" style="display: none;" 
                ...
                @forelse($tags as $tag)
                <div class="group bg-slate-900/60 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:bg-slate-900/80 hover:border-indigo-500/50 shadow-xl transition-all duration-300 relative overflow-hidden">
                    {{-- Decorative Glow --}}
                    <div class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 rounded-full blur-2xl group-hover:opacity-100 opacity-50 transition" style="background-color: {{ $tag->color }}20;"></div>
                ...
                @empty
                <div class="col-span-full rounded-2xl border border-white/10 bg-slate-900/60 shadow-xl backdrop-blur-sm px-6 py-12 text-center">
        ...
        <div x-show="showCreateModal" style="display: none;" 
            ...
            <div class="bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative" @click.away="showCreateModal = false">
                    ...
                    <form action="{{ route('admin.tags.store') }}" method="POST" class="space-y-5">
                        ...
                        <div x-data="{ color: '#3B82F6' }">
                            ...
                            <div class="flex gap-3">
                                ...
                                <input type="text" name="color_hex" x-model="color" class="flex-1 bg-slate-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-white font-mono text-sm uppercase focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition outline-none">
                            </div>
                        </div>
        ...
        <div x-show="showEditModal" style="display: none;" 
            ...
            <div class="bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative" @click.away="showEditModal = false">
                    ...
                    <form :action="editForm.action" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="flex gap-3">
                                ...
                                <input type="text" name="color_hex" x-model="editForm.color" class="flex-1 bg-slate-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-white font-mono text-sm uppercase focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition outline-none">
                            </div>
                        </div>
