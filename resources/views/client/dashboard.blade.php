<x-app-layout>
    ...
    <div x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 300)">
        ...
        <div x-show="!loaded" class="space-y-8 animate-pulse">
            {{-- Banner Skeleton --}}
            <div class="h-48 bg-slate-900/50 rounded-3xl w-full border border-white/5"></div>
            
            {{-- Stats Grid Skeleton --}}
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                @foreach(range(1, 4) as $i)
                    <div class="h-32 bg-slate-900/50 rounded-2xl border border-white/5"></div>
                @endforeach
            </div>
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-4">
                    ...
                    @foreach(range(1, 3) as $i)
                        <div class="h-24 bg-slate-900/50 rounded-2xl border border-white/5"></div>
                    @endforeach
                </div>
                <div class="space-y-4">
                    <div class="h-8 w-32 bg-white/5 rounded"></div>
                    <div class="h-64 bg-slate-900/50 rounded-2xl border border-white/5"></div>
                </div>
            </div>
        </div>
