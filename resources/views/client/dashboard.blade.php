<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-xl text-white leading-tight">Visão Geral</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 400)">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div x-show="!loaded" class="space-y-8 animate-pulse">
                <div class="h-40 bg-white/5 rounded-3xl w-full border border-white/10"></div>
                ...
                <div class="grid lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 h-96 bg-white/5 rounded-3xl border border-white/10"></div>
                    <div class="h-96 bg-white/5 rounded-3xl border border-white/10"></div>
                </div>
            </div>
            ...
            <div x-show="loaded" style="display: none;" class="space-y-8"
                ...
                {{-- 1. 🔔 ALERTA DE AÇÃO (Urgência elegante) --}}
                @if(isset($waitingForUser) && $waitingForUser > 0)
                    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 p-1 shadow-xl">
                        <div class="absolute inset-0 bg-amber-500/5 animate-pulse"></div>
                        <div class="relative flex flex-col md:flex-row items-center justify-between gap-6 p-6 rounded-[20px] bg-slate-900/60 backdrop-blur-sm">
                            <div class="flex items-center gap-5">
                                <div class="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-amber-500/20 text-2xl border border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.3)]">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-300 mt-1">O suporte respondeu a <strong class="text-amber-400">{{ $waitingForUser }} chamado(s)</strong>. Sua interação é necessária.</p>
                                </div>
                            </div>
                            <a href="{{ route('client.tickets.index', ['status' => 'waiting_client']) }}" class="group flex items-center gap-2 whitespace-nowrap px-8 py-3 rounded-xl bg-amber-500 text-slate-900 font-bold hover:bg-amber-400 transition shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 hover:-translate-y-0.5">
                ...
                @else
                    ...
                    <div class="relative overflow-hidden rounded-3xl bg-slate-900/60 backdrop-blur-sm border border-white/10 shadow-2xl group">
                        ...
                        <div class="relative z-10 p-8 sm:p-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                            ...
                            <a href="{{ route('client.tickets.create') }}" class="group relative inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 border border-indigo-400/30 text-white font-bold transition-all duration-300 shadow-xl shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-1 overflow-hidden">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                    {{-- Card: Em Aberto --}}
                    <div class="relative overflow-hidden rounded-2xl p-6 bg-slate-900/60 backdrop-blur-sm border border-white/10 shadow-xl group hover:border-slate-500/50 transition duration-300">
                    ...
                    {{-- Card: Em Atendimento --}}
                    <div class="relative overflow-hidden rounded-2xl p-6 bg-slate-900/60 backdrop-blur-sm border border-white/10 shadow-xl group hover:border-cyan-500/50 transition duration-300">
                    ...
                    {{-- Card: Finalizados --}}
                    <div class="relative overflow-hidden rounded-2xl p-6 bg-slate-900/60 backdrop-blur-sm border border-white/10 shadow-xl group hover:border-emerald-500/50 transition duration-300">
                    ...
                    {{-- Card: FAQ / Ajuda --}}
                    <a href="{{ route('faq') }}" class="relative overflow-hidden rounded-2xl p-6 bg-indigo-600/10 backdrop-blur-sm border border-indigo-500/30 shadow-xl group hover:bg-indigo-600/20 hover:border-indigo-400 transition duration-300 cursor-pointer flex flex-col justify-center">
                ...
                <div class="grid lg:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-4">
                            @forelse($recentTickets as $ticket)
                                <a href="{{ route('client.tickets.show', $ticket) }}" 
                                   class="group relative block rounded-2xl border border-white/10 bg-slate-900/60 backdrop-blur-sm p-1 hover:bg-slate-900/80 transition-all duration-300 shadow-xl hover:shadow-black/20 hover:-translate-y-0.5 hover:border-indigo-500/30">
                                    <div class="relative flex items-center justify-between p-4 sm:p-5">
                                        <div class="flex items-center gap-4 sm:gap-5 overflow-hidden">
                                            <div class="shrink-0 h-12 w-12 rounded-xl bg-slate-800/80 border border-white/10 flex items-center justify-center text-xl shadow-inner group-hover:border-indigo-500/30 transition-colors">
                                                @php
                                                    $icon = match($ticket->category ?? 'other') {
                                                        'hardware' => '🖥️', 'software' => '💾', 'network' => '🌐', 'access' => '🔒', 'printer' => '🖨️', default => '📌'
                                                    };
                                                @endphp
                                                {{ $icon }}
                                            </div>
                                            <div class="min-w-0">
                                                <div class="flex items-center gap-2 mb-1">
                                                    <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-950/50 text-slate-400 border border-white/10 group-hover:text-indigo-300 transition">
                                                        #{{ $ticket->id }}
                                                    </span>
                                                ...
                                                <h4 class="text-base font-bold text-slate-200 group-hover:text-indigo-400 truncate transition-colors">
                                                    {{ $ticket->subject }}
                                                </h4>
                                            </div>
                                        </div>
                                        <div class="shrink-0 pl-4 border-l border-white/10">
                                            <x-ticket-status :status="$ticket->status" />
                                        </div>
                                    </div>
                                </a>
                            @empty
                                <div class="flex flex-col items-center justify-center py-12 px-4 rounded-2xl border border-dashed border-white/10 bg-slate-900/60 backdrop-blur-sm text-center">
                                    <div class="h-16 w-16 bg-slate-800/80 rounded-full flex items-center justify-center mb-4 text-3xl border border-white/10">🎫</div>
                                    <h3 class="text-lg font-bold text-white mb-1">Nenhuma atividade recente</h3>
                                    <p class="text-slate-400 text-sm max-w-xs mx-auto mb-6">Seus últimos chamados aparecerão aqui para acesso rápido.</p>
                                    <a href="{{ route('client.tickets.create') }}" class="text-sm font-bold text-indigo-400 hover:text-indigo-300 hover:underline">Abrir primeiro chamado</a>
                                </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="rounded-3xl bg-slate-900/60 border border-white/10 p-6 backdrop-blur-sm shadow-xl">
                            <ul class="space-y-4">
                                @forelse($faqs as $faq)
                                    <li>
                                        <a href="{{ route('faq') }}#faq-{{ $faq->id }}" class="group flex items-start gap-3">
                                            <div class="mt-1.5 shrink-0 h-2 w-2 rounded-full bg-indigo-500/50 group-hover:bg-indigo-400 transition-colors shadow-[0_0_8px_rgba(99,102,241,0.5)]"></div>
                                ...
                                @empty
                                    <li class="text-sm text-slate-500 italic text-center py-4">Nenhuma pergunta frequente cadastrada no momento.</li>
                                @endforelse
                            </ul>
                            <div class="mt-8 pt-6 border-t border-white/10">
                                <a href="{{ route('faq') }}" class="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-white/10 text-sm font-bold text-white transition hover:shadow-lg hover:border-indigo-500/30">
                        ...
                        {{-- Card Extra: Contato --}}
                        <div class="rounded-3xl bg-slate-900/60 backdrop-blur-sm border border-white/10 p-6 text-center shadow-xl relative overflow-hidden">
                            ...
                            <div class="relative z-10">
                                ...
                                <a href="{{ route('contact') }}" class="text-sm text-indigo-400 hover:text-indigo-300 font-bold hover:underline">
