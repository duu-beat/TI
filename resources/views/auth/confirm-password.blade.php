<x-auth.brand-layout
    title="{{ __('auth.ui.confirm_password_title') }}"
    subtitle="{{ __('auth.ui.security') }}"
    headline="{{ __('auth.ui.confirm_headline') }}"
    description="{{ __('auth.ui.confirm_description') }}"
    panelHint="{{ __('auth.ui.confirm_hint') }}"
>
    ...
    <form method="POST" action="{{ route('password.confirm') }}" class="space-y-4">
        ...
        <div>
            <label for="password" class="text-sm text-slate-300">{{ __('auth.ui.password') }}</label>
