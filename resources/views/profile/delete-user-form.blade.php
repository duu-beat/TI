<x-action-section>
    <x-slot name="title"><span class="text-red-400 font-bold">{{ __('Excluir Conta') }}</span></x-slot>
    <x-slot name="description"><span class="text-slate-400">{{ __('Excluir permanentemente sua conta.') }}</span></x-slot>
    ...
    <x-slot name="content">
        <div class="rounded-2xl border border-red-500/10 bg-red-500/5 p-5">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h4 class="text-white font-bold text-base">{{ __('Atenção') }}</h4>
        ...
        <x-dialog-modal wire:model.live="confirmingUserDeletion">
            ...
            <x-slot name="content">
                <div class="text-slate-300">{{ __('Tem certeza de que deseja excluir sua conta? Digite sua senha para confirmar.') }}</div>
